...
public class TeamLeadService {
    ...
    private String generateStaffId(String branch, StaffCategory category) {
        ...
        long count = officeStaffRepository.nextBusinessId();
    ...
    public Map<String, String> createStaff(String authHeader, OfficeStaff staff) {
        ...
        PasswordPolicy.validate(staff.getPassword());
    ...
    // GET ALL STAFFS
    public Page<OfficeStaffResponseDTO> getAllStaff(Pageable pageable) {
        return officeStaffRepository.findAll(pageable).map(this::toOfficeStaffResponse);
    }

    private OfficeStaffResponseDTO toOfficeStaffResponse(OfficeStaff staff) {
        ...
        dto.setCategory(staff.getCategory() != null ? staff.getCategory().name() : null);
    ...
    public Map<String, String> createCollegeStaff(String authHeader, CollegeStaff collegeStaff) {
        ...
        if(collegeStaff.getPassword() == null || collegeStaff.getPassword().isBlank()){
            collegeStaff.setPassword(passwordEncoder.encode(PasswordPolicy.generateTemporaryPassword()));
        }else{
            PasswordPolicy.validate(collegeStaff.getPassword());
    ...
    // UNCHANGED GET METHOD
    public Page<CollegeStaff> getAllCollegeStaff(Pageable pageable) {
        return collegeStaffRepository.findAll(pageable);
    }
    ...
    public Map<String, String> updateCollegeStaff(String authHeader, Long collegeStaffId, CollegeStaff collegeStaff) {
        ...
        if (collegeStaff.getPassword() != null) {
            PasswordPolicy.validate(collegeStaff.getPassword());
            existing.setPassword(passwordEncoder.encode(collegeStaff.getPassword()));
        }
    ...
    public Page<Student> getAllStudents(Long teamLeadId, Pageable pageable) {
    ...
    // UNCHANGED GET METHOD
    public Page<OfficeStaffLeave> getAllOfficeStaffLeaves(Pageable pageable) {
        return officeStaffLeaveRepository.findAll(pageable);
    }
    ...
    */
    ...
    public Map<String,String> markHolidayODForTeamLeads(LocalDate holidayDate) {
        ...
        do {
            teamLeadPage = repository.findAll(PageRequest.of(page++, 200));
            for (TeamLead teamLead : teamLeadPage.getContent()) {
